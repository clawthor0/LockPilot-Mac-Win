...
1. Edit files in `packages/shared-ui/`
...
1. Edit only under `apps/desktop-mac/src-tauri/` (or mac-specific config files)
2. Do **not** edit `packages/shared-ui/` unless UI is intended for both platforms.
3. Commit + push to `dev`.
4. Validate mac artifact from CI.

## 3) Windows backend-only update

1. Edit only under `apps/desktop-windows/src-tauri/` (or windows-specific config files)
